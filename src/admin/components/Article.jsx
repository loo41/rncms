...
import {Input, Select, Switch, Upload, Icon, Modal, Radio, message } from 'antd'
...
class CreateArticle extends Component{
  ...
  _articleUpload = () => {
    ...
    const {masterGraph, title, subTitle, articleType, type, label} = article
    if (masterGraph === '' || title === '' || subTitle === '' || articleType === '' || type === '' || label === '') {
  ...
  _clear = () => {
    let {editor, article, banner} = this.state
    article.masterGraph = ''
    article.title = ''
    article.subTitle = ''
    article.summary = ''
    ...
    article.keyword = []
    banner.previewVisible = false
    banner.previewImage = ''
    this.setState({article, banner})
    editor.txt.html('')
  }
  _handleChange = (info) => {
    let {banner, article} = this.state
    banner.previewVisible = true
    banner.previewImage = baseURL + '/' +info.url
    article.masterGraph = baseURL + '/' +info.url
    this.setState({banner, article})
  }
  _hideModal = () => {
    let {banner} = this.state
    banner.previewVisible = false
    this.setState({banner, dataVisible: false})
  }
  componentDidMount() {
    this._setEditor()
  }
  ...
  _cheUpdate = async () => {
    ...
    let {banner, defaultShow} = this.state
    if (articleInfo && update) {
      banner.previewImage = articleInfo.masterGraph
      banner.previewVisible = true
  ...
  render() {
    let { article, type, label, banner, dataVisible, update, defaultShow} = this.state
    const {previewVisible, previewImage, fileList} = banner
    const uploadButton = (
      <div>
        <Icon type="plus" style={previewImage? {display: 'none'}: {display: 'block'}}/>
        <div className="ant-upload-text" style={previewImage? {display: 'none'}: {display: 'block'}}>Upload</div>
        <img alt="banner" src={previewImage} style={previewImage? {display: 'block', width: '100%'}: {display: 'none'}}/>
      </div>
    )
    return (
      <div className="create-article-box">
        ...
        <div className="row-box">
          ...
          <div>
            ...
            <Modal visible={previewVisible} footer={null} onCancel={this._hideModal}>
